...
def get_playlist_tracks(conn, playlist_name):
    """Return rows for all tracks on the named playlist, ordered by position."""
    query = """
        SELECT t.title,
               a.name,
               t.duration_seconds,
...
def get_tracks_on_no_playlist(conn):
    """Return rows for all tracks that do not appear on any playlist."""
    query = """
        SELECT t.track_id,
               t.title,
               a.name
...
def get_playlist_durations(conn):
    """Return each playlist's name and total duration in minutes, ordered by duration descending."""
    query = """
